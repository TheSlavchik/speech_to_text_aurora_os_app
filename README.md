...
"Voice Notes" — an offline voice recorder with on-device speech-to-text transcription, developed in C++/Qt (Sailfish Silica) for running on Aurora OS.
...
- Open `ru.omstu.voicenotes.pro` in Qt Creator and select the `AuroraOS-…-aarch64` (device) or `AuroraOS-…-x86_64` (emulator) kit  
...
- Put a Vosk speech model (e.g. [`vosk-model-small-ru`](https://alphacephei.com/vosk/models)) on the device into `~/.local/share/ru.omstu.voicenotes/model` or bundle it into the package at `/usr/share/ru.omstu.voicenotes/model`  
